...
esp_err_t aj_sr04m_init(void) {
    ...
    ESP_RETURN_ON_ERROR(gpio_config(&trigger_cfg), AJ_SR04M_TAG, "Unable to configure trigger pin");
    ESP_RETURN_ON_ERROR(
    ...
    ESP_RETURN_ON_ERROR(
      rmt_new_rx_channel(&rx_chan_cfg, &s_rx_channel),
      AJ_SR04M_TAG,
      "Unable to allocate RMT RX channel");
    ...
    ESP_RETURN_ON_ERROR(
      ...
      AJ_SR04M_TAG,
      "Unable to register RMT RX callback");

    ESP_RETURN_ON_ERROR(rmt_enable(s_rx_channel), AJ_SR04M_TAG, "Unable to enable RMT channel");
#else
    ...
    ESP_RETURN_ON_ERROR(uart_param_config(s_uart_num, &uart_config), AJ_SR04M_TAG, "Unable to configure UART");
    ESP_RETURN_ON_ERROR(
      uart_set_pin(s_uart_num, AJ_SR04M_TRIGGER_PIN, AJ_SR04M_ECHO_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE),
      AJ_SR04M_TAG,
      "Unable to set UART pins");
#endif
    return ESP_OK;
}
...
void aj_sr04m_trigger(void) {
    ...
    rmt_receive_config_t rx_cfg = {
      ...
      .signal_range_max_ns = AJ_SR04M_RMT_IDLE_NS,  /* idle threshold = end of frame */
    };
...
aj_sr04m_dist_status_t aj_sr04m_read_duration(int16_t* distance) {
#if AJ_SR04M_MODE <= 2
    if (xSemaphoreTake(s_rx_done_sem, pdMS_TO_TICKS(AJ_SR04M_RMT_TIMEOUT_MS)) != pdTRUE)
        return AJ_SR04M_DIST_NO_ECHO;
    ...
    *distance = mm;
    ...
    char data[64];
    int len = uart_read_bytes(s_uart_num, (uint8_t*)data, sizeof(data) - 1, pdMS_TO_TICKS(250));
    if (len <= 0)
        return AJ_SR04M_DIST_BAD_FRAME;
    data[len] = '\0';
