 ...
 *
 * Configured via `idf.py menuconfig` → AJ-SR04M Configuration → Sensor operating mode.
 *
 * Available values:
 ...
 *    - 5: UART low-power, ASCII frame "Gap=XXXX mm" emitted after a UART trigger byte
 */
 ...
 *
 * Frame format: 0xFF | dist_H | dist_L | checksum, with checksum = (0xFF + dist_H + dist_L) & 0xFF.
 *
 * @param[in]  data     pointer to the frame buffer
 * @param[in]  len      number of bytes in @p data (must be 4 for a valid frame)
 * @param[out] distance distance in millimeters (valid only if return is AJ_SR04M_DIST_OK)
 *
 * @return
 *    - AJ_SR04M_DIST_OK if the frame is valid and the distance is in [200, 4500] mm
 *    - AJ_SR04M_DIST_BAD_FRAME if @p len is not 4 or the header byte is not 0xFF
 ...
 */
aj_sr04m_dist_status_t aj_sr04m_parse_binary_frame(const uint8_t* data, int len, int16_t* distance);

/**
 * @brief Parse an ASCII frame from the AJ-SR04M (mode 5).
 *
 * Frame format: "Gap=XXXX mm" possibly preceded by garbage bytes and followed by CR/LF.
 *
 * @param[in]  data     NUL-terminated buffer containing the frame
 * @param[out] distance distance in millimeters (valid only if return is AJ_SR04M_DIST_OK)
 *
 * @return
 *    - AJ_SR04M_DIST_OK if the "Gap=...mm" pattern is found and the distance is in [200, 4500] mm
 *    - AJ_SR04M_DIST_BAD_FRAME if the pattern is missing or unparseable
 *    - AJ_SR04M_DIST_NO_ECHO if the parsed distance is out of [200, 4500] mm
 */
aj_sr04m_dist_status_t aj_sr04m_parse_ascii_frame(const char* data, int16_t* distance);

/**
 * @brief Initialize the AJ-SR04M sensor.
 *
 * Configures the trigger GPIO + RMT RX capture (modes 1 and 2) or the UART (modes 3, 4 and 5)
 * according to AJ_SR04M_MODE.
 *
 * @return
 *    - ESP_OK on success
 ...
 *
 * @param[out] distance distance in millimeters (valid only if the return value is AJ_SR04M_DIST_OK)
 *
 * @return
