 ...
 *
 * Same call sequence as mode 4 (UART init, trigger byte write, then read),
 * but the response is parsed by `aj_sr04m_parse_ascii_frame`. The variant
 * assertion is the same shape — Kconfig wires 0x01 / 0x55 based on AJ-SR04M
 * vs JSN-SR04T.
 */

#include "sdkconfig.h"

#if CONFIG_AJ_SR04M_MODE_5 && CONFIG_AJ_SR04M_MAX_SENSORS == 1
...
TEST_CASE("read: mode 5 valid ASCII frame at 1500 mm", "[aj_sr04m][read]") {
  mocks_reset();
...
TEST_CASE("read: mode 5 BAD_FRAME on missing Gap= pattern",
          "[aj_sr04m][read]") {
  mocks_reset();
...
TEST_CASE("read: mode 5 NO_ECHO on out-of-range (6016 mm)",
          "[aj_sr04m][read]") {
  mocks_reset();
...
TEST_CASE("read: mode 5 BAD_FRAME on UART read returning 0 bytes",
          "[aj_sr04m][read]") {
  mocks_reset();
